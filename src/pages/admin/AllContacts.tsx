...
import type { ContactProps } from '../../lib/sharedInterface';
...
const AllContacts: React.FC = () => {
    ...
    const fetchAllContacts = async () => {
        setIsLoading(true)
        try {
          const response = await api.get(`/contact?per_page=${apiItemsPerPage}`, {
            headers: { 
        ...
        <div>
            <div className="overflow-x-auto rounded-lg no-scrollbar w-full lg:p-0 pe-4">
                ...
                <thead>
                    ...
                    <th className='p-4 md:text-sm text-xs whitespace-nowrap'>Message</th>
                    <th className='p-4 md:text-sm text-xs whitespace-nowrap'>Date Sent</th>
                ...
                <tbody>
                    {isLoading ? (
                        <tr className='bg-white/61 border-y border-black/10'>
                            <td colSpan={7} className="text-center py-4 bg-white">Fetching contacts...</td>
                        </tr>
                    )  : allContacts.length === 0 && !isLoading ? (
                        <tr>
                            <td colSpan={7} className="text-center bg-white/61 py-4 border-y border-black/10">No contacts found.</td>
                        </tr>
                    ) : (
