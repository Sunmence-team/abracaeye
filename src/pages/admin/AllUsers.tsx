  ...
  const fetchAllUsers = async () => {
    ...
    try {
      const response = await api.get(
        `/users?per_page=${apiItemsPerPage}&page=${currentPage}`,
        {
          headers: {
        ...
        <table className="w-full min-w-[800px] text-center">
          <thead>
            <tr className="bg-dark-red text-white h-[55px]">
              ...
              <th className="p-4 text-[10px] whitespace-nowrap">Name</th>
              <th className="p-4 text-[10px] whitespace-nowrap">Email</th>
              <th className="p-4 text-[10px] whitespace-nowrap">Is Blogger</th>
              <th className="p-4 text-[10px] whitespace-nowrap">Is Vendor</th>
              <th className="p-4 text-[10px] whitespace-nowrap">Date Joined</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-[10px] font-medium">
                        <button
                          type="button"
                          className="bg-dark-red text-white rounded-md text-xs h-[40px] px-2 disabled:opacity-50 cursor-pointer"
                          disabled={user?.blog || isVerifying}
                          onClick={() => handleVerifyUser(user)}
                        >
                          {isVerifying ? "Verifing..." : "Verify as blogger"}
                        </button>
                      </td>
                    </tr>
                  );
